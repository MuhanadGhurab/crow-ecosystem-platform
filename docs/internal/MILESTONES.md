...
|----|-----------|------------------|-------|---|--------|
| **C3.1** | Crow brand & homepage identity (no paid infra) | Canonical mark system, hero background, loader continuity, Architecture Lab brand section, `crow-brand-identity:verify`, `crow-motion-identity:verify` | Muhanad | **~95%** | **Conditional pass** — **CONDITIONAL PASS — HERO ART DIRECTION APPROVAL REQUIRED** — PO visual review pending — no commit/push — [`C3_CROW_BRAND_AND_HOMEPAGE_IDENTITY.md`](../architecture/crow-core/c3/C3_CROW_BRAND_AND_HOMEPAGE_IDENTITY.md) |
| **C2.2** | Database isolation & migration control (no paid infra) | Remove build-time migrate, fingerprint/env model, controlled migration CLI + GHA workflow, C2 mutation guard, ops docs, `c2-database-isolation:verify` | Muhanad | **~95%** | **Conditional pass** — **CONDITIONAL PASS — BUILD MIGRATION REMOVED, EXTERNAL ISOLATION PENDING** — branch `feat/c2-2-database-isolation-migration-control` stacked on C2.1 — uncommitted — [`C2_2_DATABASE_ISOLATION_MIGRATION_CONTROL.md`](C2_2_DATABASE_ISOLATION_MIGRATION_CONTROL.md) |
| **C2.1** | Preview migration readiness gate (no paid infra) | Read-only Preview DB audit, isolation fingerprint, migration history, backfill dry-run, apply runbook, `c2-preview-readiness:verify` | Muhanad | **100%** | **Blocked** — **BLOCKED — PREVIEW/PRODUCTION DATABASE ISOLATION NOT PROVEN** — branch `feat/c2-1-preview-migration-readiness` stacked on C2 — PR #7 — no hosted apply in C2.1 — [`C2_1_PREVIEW_MIGRATION_READINESS_GATE.md`](C2_1_PREVIEW_MIGRATION_READINESS_GATE.md) |
| **C2** | Blueprint persistence runtime (no paid infra) | Hybrid Path C Prisma + migration #14, versioning/approval/trace/ROI/SOW runtime, tenant auth, dual-read, backfill dry-run, `c2-blueprint-runtime:verify` | Muhanad | **~95%** | **Passed (local)** — **PASSED — READY FOR PREVIEW MIGRATION REVIEW** — branch `feat/c2-blueprint-persistence-runtime` stacked on C1.1 — PR #6 — [`C2_BLUEPRINT_PERSISTENCE_RUNTIME.md`](C2_BLUEPRINT_PERSISTENCE_RUNTIME.md) |
| **C1.1** | Blueprint persistence migration approval gate (no paid infra) | Hybrid architecture, auth matrix, threat model, backfill/rollout, schema preview (non-executable), `c1-migration-gate:verify`, PO sign-off | Muhanad | **100%** | **Approved** — **APPROVE PATH C — READY FOR C2** — branch `feat/c1-1-blueprint-persistence-gate` stacked on C1 — PR #5 — [`C1_1_BLUEPRINT_PERSISTENCE_MIGRATION_APPROVAL_GATE.md`](C1_1_BLUEPRINT_PERSISTENCE_MIGRATION_APPROVAL_GATE.md) |
...
| **I5** | Proposal / Blueprint authenticated review (no paid infra) | Review contract + service, `/client/proposals` + detail + blueprint review, token safety, `client-review:verify` | Muhanad | **100%** | **Passed** — [`I5_PROPOSAL_BLUEPRINT_AUTHENTICATED_REVIEW.md`](I5_PROPOSAL_BLUEPRINT_AUTHENTICATED_REVIEW.md) |
...
**Scope:** Read-only authenticated proposal and blueprint review inside `/client` — review contract, review service, enriched list/detail pages, request integration, public token safety, and `client-review:verify` — without approval mutations, schema migrations, or paid infra.

**Status:** **Passed** (27 May 2026)

| Deliverable | Location |
|-------------|----------|
| Master record | [`I5_PROPOSAL_BLUEPRINT_AUTHENTICATED_REVIEW.md`](I5_PROPOSAL_BLUEPRINT_AUTHENTICATED_REVIEW.md) |
| Review contract | `src/lib/client-portal/client-review-contract.ts` |
| Service | `src/lib/services/client-review.service.ts` |
...
## F35 — Aviation operating model depth & Najm intake hardening (no paid infra)

**Scope:** Deepen **Aviation** as a first-class Crow operating model and validate **Najm-style organic intake** assumptions (F11) — without paid infra, live payments, schema changes, Najm tenant provisioning, or public customer claims.

**Status:** **Passed** (26 May 2026).

| Deliverable | Location |
|-------------|----------|
| Master plan | [`F35_AVIATION_OPERATING_MODEL_DEPTH.md`](F35_AVIATION_OPERATING_MODEL_DEPTH.md) |
| Sector template | `src/lib/org-intelligence/sector-template-data.ts` (`AVIATION`) |
| Discovery guidance | `sector-guidance.ts` · `discovery-templates/aviation.json` |
| Verification | `npm run aviation:verify` → `scripts/verify-aviation-sector-template.ts` |
| Najm payload | `scripts/f11-najm-payload.json` (industry `aviation`, synthetic notes) |
| DB seed (optional) | `npm run db:seed:sectors` (idempotent; operator-approved) |

**Acceptance:** Audit documented · template with 12 depts / 15 roles / 12 workflows · live CEM modules only · SAREA + CyberCrow models · Najm organic path validated · no logistics/retail/construction leakage · validation green.

---

## F34 — Construction operating model depth & Rimal hardening (no paid infra)

**Scope:** Deepen **Construction** as a first-class Crow operating model and validate **Rimal** staging tenant alignment — without paid infra, live payments, schema changes, or public customer claims.

**Status:** **Passed** (26 May 2026).

| Deliverable | Location |
|-------------|----------|
| Master plan | [`F34_CONSTRUCTION_OPERATING_MODEL_DEPTH.md`](F34_CONSTRUCTION_OPERATING_MODEL_DEPTH.md) |
| Sector template | `src/lib/org-intelligence/sector-template-data.ts` (`CONSTRUCTION`) |
| Discovery guidance | `sector-guidance.ts` · `discovery-templates/construction.json` |
| Verification | `npm run construction:verify` → `scripts/verify-construction-sector-template.ts` |
| Rimal checks | `npm run tenant:verify:rimal` (staging DB, read-only) |
| DB seed (optional) | `npm run db:seed:sectors` (idempotent; operator-approved) |

**Acceptance:** Audit documented · template with 12 depts / 16 roles / 16 workflows · live CEM modules only · SAREA + CyberCrow models · Rimal module ⊆ construction recommendations · no logistics/retail leakage · validation green.
...
## F31 — Workspace hygiene & release cleanliness (no paid infra)

**Scope:** Post-F30 git audit — classify modified/untracked files, forbidden-file check, straggler mapping, validation baseline, `.gitignore` review, tag readiness. No commits, tag, migrations, seeds, or paid infra in this phase.

**Status:** **Passed with warnings** (26 May 2026).

| Deliverable | Location |
|-------------|----------|
| Master plan | [`F31_WORKSPACE_HYGIENE_RELEASE_CLEANLINESS.md`](F31_WORKSPACE_HYGIENE_RELEASE_CLEANLINESS.md) |
...
**Scope:** Deepen CEM operational visibility — workflow intelligence service, workflows/tasks/departments/roles pages, dashboard ops panel, reports readiness, admin CEM tab, CyberCrow/SAREA linkage — without schema changes, paid infra, or automation-engine claims.

**Status:** **Passed** (25 May 2026).

| Deliverable | Location |
|-------------|----------|
| Master plan | [`F26_CEM_WORKFLOW_OPERATIONS_DEPTH.md`](F26_CEM_WORKFLOW_OPERATIONS_DEPTH.md) |

**Validation:** `typecheck`, `lint`, `build`, `public:mirror-manifest`, `meem:ids:staging`, `tenant:verify:rimal`, `request:pipeline:verify`.

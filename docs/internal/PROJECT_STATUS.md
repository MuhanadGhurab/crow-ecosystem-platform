...
**Last updated:** 5 Jun 2026 (M4C tenant invite acceptance — **PROPOSAL-ONLY PASS**; M4B tenant invite/onboarding — **PASSED**; M3.5 purchase-to-stock manual smoke — **CONDITIONAL PASS**; M4 tenant membership — **PASSED**; production deploy **Ready** after M4B)  
...
**M4C acceptance:** **PROPOSAL-ONLY PASS** — [`M4C_TENANT_INVITE_ACCEPTANCE_TOKEN_EMAIL_DELIVERY.md`](M4C_TENANT_INVITE_ACCEPTANCE_TOKEN_EMAIL_DELIVERY.md) · [`M4C_TENANT_INVITE_ACCEPTANCE_SCHEMA_PROPOSAL.md`](M4C_TENANT_INVITE_ACCEPTANCE_SCHEMA_PROPOSAL.md).
Schema audit: `TenantMembership` cannot store invite token/status/expiry; new `TenantMembershipInvite` table proposed (token hash only, status lifecycle, acceptance creates membership). **No migration or implementation** until explicit approval. Email: manual copy-link default; M4D for provider. `npm run tenant-invite-acceptance:verify` + M4B/M4 verifier regression.

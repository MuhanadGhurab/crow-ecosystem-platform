...
**M4C.1.1 acceptance:** **CONDITIONAL PASS** — [`M4C_1_1_INVITE_ACCEPTANCE_OPERATOR_SMOKE_COMPLETION.md`](M4C_1_1_INVITE_ACCEPTANCE_OPERATOR_SMOKE_COMPLETION.md).
M4C.1.1 re-ran full verifier suite on branch `b575c9c` (A1 + A1.1 + M4C); production health **PASS**; operator browser smoke Parts 2–9 **NOT EXECUTED** (Browse MCP `ENOENT`, no platform/test credentials). Staging migration `20260605120000_tenant_membership_invite` **not SQL-verified** on `wbwnsndcxrgyqwppurms`. Local uncommitted fix: reserve `tenant-invite` in `RESERVED_PATH_SEGMENTS` so signed-out users reach public invite page (not tenant auth redirect). **FULL PASS blocked** until operator completes manual checklist on preview (with bypass) or post-merge URL.

...
**Last updated:** 29 May 2026 (M2.0 Vercel Build OOM Optimization)  
...
**M2.0 acceptance:** **PASSED** — [`M2_0_VERCEL_BUILD_OOM_OPTIMIZATION.md`](M2_0_VERCEL_BUILD_OOM_OPTIMIZATION.md).
Vercel SIGKILL/OOM after M2 traced to `next build` memory (not DB/migrations); `webpackMemoryOptimizations` + disabled prod source maps + build heap wrapper; M2 import graph split (studio loader, lightweight Go/No-Go module, contract types); `force-dynamic` on DB-backed M2 routes; `build-memory:verify`; M2 and track verifiers unchanged; no auth/payment/provisioning weakening or `ignoreBuildErrors`.

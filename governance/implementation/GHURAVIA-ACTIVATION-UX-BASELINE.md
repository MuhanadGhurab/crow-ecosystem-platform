...
- Shared activation shell, progress, Explainable Locks, localized errors, route guards
- Playwright + `@axe-core/playwright` (dev-only); Critical/Serious axe violations **0**
- Keyboard activation e2e **PASS**; route guards **PASS**
- Synthetic session, disposable Postgres, email mock mailbox, outbox worker (inherited from 0B)

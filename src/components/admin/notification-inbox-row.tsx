    ...
    >
      ...
      <div className="flex w-full flex-wrap items-start justify-between gap-2">
        ...
        {showActions && !compact && (
          ...
          <NotificationStatusActions notificationId={row.id} inboxStatus={row.parsed.inboxStatus} />
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500">
        ...
        <span
          className={
            row.parsed.deliveryStatus === "sent"
              ? "text-teal-300"
              : row.parsed.deliveryStatus === "failed"
                ? "text-red-400"
                : row.parsed.inboxStatus === "reviewed"
                  ? "text-cyan-300"
                  : row.parsed.inboxStatus === "dismissed"
                    ? "text-slate-500"
                    : "text-slate-400"
          }
          title="Delivery / inbox"
        >
          {row.parsed.deliveryStatus}

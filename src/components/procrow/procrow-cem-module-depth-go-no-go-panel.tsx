import type { CemModuleDepthGoNoGoDependency } from "@/lib/cem/cem-module-depth-contract";

type Props = {
  dependency: CemModuleDepthGoNoGoDependency;
};

const STATUS_STYLE: Record<CemModuleDepthGoNoGoDependency["status"], string> = {
  ready: "border-teal-500/30 text-teal-200",
  warning: "border-amber-500/30 text-amber-200",
  blocked: "border-rose-500/30 text-rose-200",
};

export function ProCrowCemModuleDepthGoNoGoPanel({ dependency }: Props) {
  return (
    <section className="cc-glass-card cc-entity-block--cem !p-5 space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <h2 className="font-display text-sm font-semibold uppercase tracking-wider text-violet-300">
          CEM module depth (Go/No-Go dependency)
        </h2>
        <span
      ...
      <ul className="list-disc pl-4 text-xs text-slate-500 space-y-1">
        {dependency.depthChecks.map((check) => (
          <li key={check}>{check}</li>
        ))}
      </ul>
      <p className="text-xs text-slate-500">
        Does not auto-pass Go/No-Go — F23-gated production; no payment or subscription activation.
      </p>
    </section>
  );
}

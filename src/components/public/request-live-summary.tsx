    ...
    >
      ...
      <div className={compact ? "flex items-end justify-between gap-4" : "mt-5 space-y-3"}>
        <div className={compact ? "min-w-0" : ""}>
          <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">Monthly (indicative)</p>
          <p
        ...
        {!compact && (
          <dl className="grid grid-cols-3 gap-2 text-center text-xs">
            ...
            <div className="rounded-cc-sm border border-white/10 bg-white/[0.03] px-2 py-2">
              ...
              <dd className="mt-0.5 font-medium text-teal-300">+{formatSar(estimate.modulesMonthlySar)}</dd>
            </div>
            <div className="rounded-cc-sm border border-white/10 bg-white/[0.03] px-2 py-2">
              <dt className="text-slate-500">Security</dt>
              <dd className="mt-0.5 font-medium text-violet-300">+{formatSar(estimate.securityMonthlySar)}</dd>
            </div>
          </dl>
        )}
        {compact && (
      ...
      {!compact && (
        <p className="mt-4 text-xs leading-relaxed text-slate-500">
          Indicative pricing (excl. VAT) — 15% VAT applied at invoice. Final commercial terms follow

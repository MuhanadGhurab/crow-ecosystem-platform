...
export const ERP_MODULE_CATALOG: ErpModuleCatalogEntry[] = [
  ...
  },
  {
    ...
    hasTenantRoute: true,
    category: "core_erp",
    shortDescription: "People records linked to departments and roles.",
    businessPurpose: "Anchor org structure for access, SAREA mapping, and workforce reporting.",
    primaryUsers: ["HR admin", "People ops", "Department manager"],
    relatedDepartments: ["HR", "People operations"],
    ownedDataExamples: ["Employee profiles", "Department assignment", "Role linkage"],
    commonWorkflows: ["Onboarding", "Transfer", "Offboarding (advisory)"],
    approvalNeeds: ["Role change", "Sensitive profile edit"],
    reportSignals: ["Headcount", "Open positions (advisory)"],
    cyberCrowRisks: ["Orphan accounts after offboarding", SHARED_CYBER.access],
    evidenceExamples: ["Termination checklist", "Role assignment approval"],
    auditEvents: ["hr.employee.updated", "hr.role.assigned"],
    sareaExperienceHints: ["Employee self-service: minimal", "HR admin: full forms"],
    sectorRelevance: rel(
      ["logistics", "retail", "construction", "aviation", "healthcare"]
    ),
    dependencies: ["departments", "roles", "users"],
    implementationStatus: "operational_list",
    dataSource: "hr.service + tenant identity",
    uiMaturityNotes: "Among strongest modules — employee forms and lists",
    futureDepth: ["Leave management", "Payroll export (future-only)"],
  },
  {
    cemModuleKey: "crm",
    erpKey: "crm",
